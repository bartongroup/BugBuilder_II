...
def get_quast_stats(quast_report):
    """
    ...
    """
    print(f"Extracting QUAST stats from {quast_report}\n")

    quast_df = pd.read_csv(quast_report, sep='\t', header=None)
    print("QUAST report loaded successfully.\n")
    ...
    print("QUAST stats extracted:\n")
    for key, value in stats.items():
        print(f"{key}: {value}\n")

    return stats

def get_all_quast_stats(quast_reports):
    """
...
def get_fastp_stats(fastp_json):
    ...
    """
    print(f"Extracting fastp stats from {fastp_json}\n")

    try:
        with open(fastp_json) as f:
            data = json.load(f)
            print("fastp JSON report loaded successfully.\n")
    except Exception as e:
        print(f"Error loading fastp JSON report: {e}\n")
        return {}
    ...
    print("fastp stats extracted:\n")
    for key, value in stats.items():
        print(f"{key}: {value}\n")

    return stats

def get_nanostat_stats(nanostat_report):
    """
    ...
    """
    print(f"Extracting NanoStat stats from {nanostat_report}\n")

    nanostat_df = pd.read_csv(nanostat_report, sep='\t')
    print("NanoStat report loaded successfully.\n")
    ...
    print("NanoStat stats extracted:\n")
    for key, value in stats.items():
        print(f"{key}: {value}\n")

    return stats

def get_checkm2_stats(checkm2_reports):
    """
    ...
    for report in checkm2_reports:
        sample_name = Path(report).parent.name
        print(f"Processing CheckM2 report for sample: {sample_name}\n")
        try:
            df = pd.read_csv(report, sep='\t')
            print(f"CheckM2 report for {sample_name} loaded successfully.\n")
        except Exception as e:
            print(f"Error loading CheckM2 report for {sample_name}: {e}\n")
            continue
        ...
        print(f"CheckM2 stats extracted for {sample_name}:\n")
        for key, value in stats.items():
            print(f"{key}: {value}\n")
...
def extract_RNA_seqs(bakta_dir):
    """
    ...
    for assembly in assembly_files:
        ...
        print(f"Processing assembly for sample: {sample}")
